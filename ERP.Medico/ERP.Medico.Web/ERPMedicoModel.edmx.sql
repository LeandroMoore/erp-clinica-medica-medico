...
-- --------------------------------------------------
-- Date Created: 02/22/2011 20:24:22
-- Generated from EDMX file: D:\Docs\Poli\2011\LabSoft\erp-clinica-medica-medico\ERP.Medico\Erp.Medico.Web\ERPMedicoModel.edmx
-- --------------------------------------------------
...
GO
IF OBJECT_ID(N'[dbo].[FK_PacienteAnamnese]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Pacientes] DROP CONSTRAINT [FK_PacienteAnamnese];
GO
...
CREATE TABLE [dbo].[Anamneses] (
    ...
    [DescricaoDoencaAtual] nvarchar(max)  NULL,
    [DescricaoMedicaPregressa] nvarchar(max)  NULL,
    [HistoricoFamiliar] nvarchar(max)  NULL,
    [HistoricoPessoalSocial] nvarchar(max)  NULL,
...
-- Creating table 'TiposAtendimentos'
CREATE TABLE [dbo].[TiposAtendimentos] (
    [Id] int IDENTITY(1,1) NOT NULL,
    [Descricao] nvarchar(max)  NOT NULL
);
GO

-- Creating table 'TiposSangues'
CREATE TABLE [dbo].[TiposSangues] (
    [Id] int IDENTITY(1,1) NOT NULL,
    [Descricao] nvarchar(max)  NOT NULL
);
GO
...
-- Creating primary key on [Id] in table 'TiposAtendimentos'
ALTER TABLE [dbo].[TiposAtendimentos]
ADD CONSTRAINT [PK_TiposAtendimentos]
    PRIMARY KEY CLUSTERED ([Id] ASC);
GO

-- Creating primary key on [Id] in table 'TiposSangues'
ALTER TABLE [dbo].[TiposSangues]
ADD CONSTRAINT [PK_TiposSangues]
    PRIMARY KEY CLUSTERED ([Id] ASC);
GO
...
-- Creating foreign key on [Anamnese_Id] in table 'Pacientes'
ALTER TABLE [dbo].[Pacientes]
ADD CONSTRAINT [FK_PacienteAnamnese]
    FOREIGN KEY ([Anamnese_Id])
    REFERENCES [dbo].[Anamneses]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;

-- Creating non-clustered index for FOREIGN KEY 'FK_PacienteAnamnese'
CREATE INDEX [IX_FK_PacienteAnamnese]
ON [dbo].[Pacientes]
    ([Anamnese_Id]);
GO
...
ADD CONSTRAINT [FK_AtendimentoTipoAtendimento]
    ...
    REFERENCES [dbo].[TiposAtendimentos]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
